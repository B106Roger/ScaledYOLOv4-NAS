...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.conv = lora.Conv2d(c1, c2, k, r=LORA_RANK, lora_alpha=LORA_ALPHA, stride=s, padding=autopad(k, p), groups=g, bias=False)
...
class BottleneckCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, e=0.5, shortcut=True, g=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        LORA_ALPHA = int(LORA_RANK * 2)
        self.cv2 = lora.Conv2d(c1, c_, 1, r=LORA_RANK, lora_alpha=LORA_ALPHA, stride=1, bias=False)
        # self.cv3 = nn.Conv2d(c_, c_, 1, 1, bias=False)
        self.cv3 = lora.Conv2d(c_, c_, 1, r=LORA_RANK, lora_alpha=LORA_ALPHA, stride=1, bias=False)
...
class BottleneckCSP2(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, e=0.5, shortcut=False, g=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        LORA_RANK = int(c_ // LORA_RAN_BASE)
        LORA_ALPHA = int(LORA_RANK * 2)
        self.cv2 = lora.Conv2d(c_, c_, 1, r=LORA_RANK, lora_alpha=LORA_ALPHA, stride=1, bias=False)
