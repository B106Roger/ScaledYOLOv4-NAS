...
from models.yolo_lora import Model
...
def train(hyp, opt, device, tb_writer=None):
    ...
    ema = ModelEMA(model, decay=0.8, updates=1) if rank in [-1, 0] else None
    ...
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, opt, hyp=hyp, augment=True,
                                            ...
                                            use_shared_mem=False, shared_mem_limit=4000)
    ...
    print(f"Trainable Parameters (EMA): {pytorch_total_params}")
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni % accumulate == 0:
                step_time = time.time()
                ###############################################
                # print(model.model[27])
                # print(f"blocks.27.conv.lora_A = {model.model[27].conv.lora_A.grad}")
                # print(f"blocks.27.conv.lora_B = {model.model[27].conv.lora_B.grad}")
                # print(f"blocks.27.conv.conv.weight = {model.model[27].conv.conv.weight.grad}")
                scaler.step(optimizer)  # optimizer.step
                scaler.update()
                # print(f"blocks.27.conv.marge = {model.model[27].conv.merged}")
                # print(f"blocks.27.conv.lora_A = {model.model[27].conv.lora_A.grad}")
                # print(f"blocks.27.conv.lora_B = {model.model[27].conv.lora_B.grad}")
                # print(f"blocks.27.conv.conv.weight = {model.model[27].conv.conv.weight}")
                optimizer.zero_grad()
                ###############################################
                ...
                if ema is not None:
                    ema_time = time.time()
                    ##################################
                    # print(f"blocks.27.conv.lora_A = {ema.model[27].conv.lora_A}")
                    # print(f"blocks.27.conv.lora_B = {ema.model[27].conv.lora_B}")
                    # print(f"blocks.27.conv.conv.weight = {ema.model[27].conv.conv.weight}")
                    ema.update_lora(model)                #
                    # print(f"EMA.27.conv.marge = {ema.ema.model[27].conv.merged}")
                    # print(f"blocks.27.conv.lora_A = {ema.ema.model[27].conv.lora_A}")
                    # print(f"blocks.27.conv.lora_B = {ema.ema.model[27].conv.lora_B}")
                    # print(f"blocks.27.conv.conv.weight = {ema.ema.model[27].conv.conv.weight}")
                    ##################################
        ...
        if rank in [-1, 0]:
            ...
            if save:
                ...
                torch.save(lora.lora_state_dict(ema.ema), os.path.join(wdir, f'lora_last.pt'))
                if epoch >= (epochs-10):
                    # torch.save(ckpt, last.replace('.pt','_{:03d}.pt'.format(epoch)))
                    torch.save(lora.lora_state_dict(ema.ema), os.path.join(wdir, f'lora_{epoch}.pt'))
                if best_fitness == fi:
                    # torch.save(ckpt, best)
                    torch.save(lora.lora_state_dict(ema.ema), os.path.join(wdir, f'lora_best.pt'))
                del ckpt
        # end epoch ----------------------------------------------------------------------------------------------------
    # end training
